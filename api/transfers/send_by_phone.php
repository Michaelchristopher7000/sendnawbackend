...
// Conditionally include sendFCM only if file exists
...
if (!$phoneNumber || $amount <= 0) {
    echo json_encode(['success' => false, 'message' => 'Phone number and amount required']);
    exit;
}

// Find receiver by phone
$stmt = $pdo->prepare("SELECT id, email, full_name FROM users WHERE phone = ?");
...
try {
    ...
    $description = "Transfer to phone $phoneNumber";
    ...
    // Send email alerts (inside transaction, but after commit)
    $date = date('Y-m-d H:i:s');
    sendEmail($senderEmail, "SendNaw: You sent $currency $amount to $receiverName", "<h3>Transfer Sent</h3><p>You sent <strong>$currency $amount</strong> to $receiverName (Phone: $phoneNumber).</p><p><strong>Reference:</strong> $ref</p><p><strong>Date:</strong> $date</p>");
    sendEmail($receiverEmail, "SendNaw: You received $currency $amount from $senderName", "<h3>Transfer Received</h3><p>You received <strong>$currency $amount</strong> from $senderName (Phone: $phoneNumber).</p><p><strong>Reference:</strong> $ref</p><p><strong>Date:</strong> $date</p>");

    // FCM push notification (if available)
