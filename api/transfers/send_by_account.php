...
// Conditionally include FCM helper
...
$stmt->execute([$token]);
$tokenRow = $stmt->fetch();
if (!$tokenRow) exit(json_encode(['success' => false, 'message' => 'Invalid token']));
...
if (!$accountNumber || $amount <= 0) {
    exit(json_encode(['success' => false, 'message' => 'Account number and amount required']));
}
...
$receiver = $stmt->fetch();
if (!$receiver) exit(json_encode(['success' => false, 'message' => 'Recipient not found']));
$receiverId = $receiver['id'];
$receiverEmail = $receiver['email'];
$receiverName = $receiver['full_name'];
if ($senderId == $receiverId) exit(json_encode(['success' => false, 'message' => 'Cannot send to yourself']));
...
$stmt = $pdo->prepare("SELECT u.kyc_tier, ul.single_transfer_limit FROM users u JOIN user_limits ul ON u.kyc_tier = ul.tier WHERE u.id = ?");
$stmt->execute([$senderId]);
$limitData = $stmt->fetch();
if ($amount > $limitData['single_transfer_limit']) {
    exit(json_encode(['success' => false, 'message' => 'Amount exceeds single transfer limit']));
}
...
try {
    ...
    if (!$senderWallet || $senderWallet['balance'] < $amount) throw new Exception('Insufficient balance');
    ...
    // Credit receiver
    $stmt = $pdo->prepare("INSERT INTO wallets (user_id, currency_code, balance) VALUES (?, ?, 0) ON DUPLICATE KEY UPDATE balance = balance + ?");
    ...
    sendEmail($senderEmail, "SendNaw: You sent $currency $amount to $receiverName",
        "<h3>Transfer Sent</h3><p>You sent <strong>$currency $amount</strong> to $receiverName (Account: $accountNumber).</p>
    ...
    sendEmail($receiverEmail, "SendNaw: You received $currency $amount from $senderName",
        "<h3>Transfer Received</h3><p>You received <strong>$currency $amount</strong> from $senderName (Account: {$sender['full_name']}).</p>
         <p><strong>Reference:</strong> $ref</p><p><strong>Date:</strong> $date</p>");

    // Push notification (if FCM is available)
